...
class Entrada extends Model {
	...
	public function getTotalEntradas($data_inicial, $data_final)
	{
		$array = array();
	...
	public function addReceita($descricao, $valor, $data_d, $conta, $id_cat)
	{
		...
		if ($sql->rowCount() > 0) {
            $_SESSION['alert'] = '<div class="alert alert-success">
									...
									<span>
									<b> Sucesso - </b> Entrada cadastrada com sucesso!</span>
								  </div>';
        } else {
	...
	public function addReceitaHome($descricao, $valor, $data_d, $conta, $id_cat)
	{
		$sql ="INSERT INTO receita SET descricao = :descricao, valor = :valor, data_d = :data_d,
		conta = :conta, id_cat = :id_cat";
		$sql = $this->db->prepare($sql);
	...
	{
		if ($juro == null) {
			$sql ="INSERT INTO rec_recorrente SET descricao = :descricao, valor = :valor, ventrada = :ventrada, juro = :juro, qtd_parc = :qtd_parc, data_parc = :data_parc, conta = :conta, id_cat = :id_cat";
			$sql = $this->db->prepare($sql);
			$sql->bindValue(':descricao', $descricao);
			$sql->bindValue(':valor', $valor);
