	...
	public function verifyLogin()
	{
		if (!empty($_SESSION['uLogin'])) {
			...
			if ($sql->rowCount() > 0) {
				...
				$sqlc ="SELECT * FROM companies INNER JOIN store_info ON(companies.id = store_info.id_company) WHERE companies.id = :company";
				...
				if($sqlc->rowCount() > 0) {
					...
					$data['company_name'] = $company['name_company'];
                    setUser($data);
					setStore($company);
				}
...
//        exit;
	...
	public function newPass($password, $id_user)
	{
		...
		$sql ="UPDATE usuarios SET password = :se WHERE id = :id";
		$sql = $this->db->prepare($sql);
		$sql->bindValue(':se', $passhash);
